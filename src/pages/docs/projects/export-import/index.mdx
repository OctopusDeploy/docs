...
:::div{.warning}
GitHub connection used by a version-controlled project will not be exported, due to that the fact that we can't re-configure GitHub connections on the destination side, because IDs and other details are all instance-specific on the GitHub App side.
:::
