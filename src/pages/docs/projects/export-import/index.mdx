...
:::div{.warning}
GitHub connection used by a version-controlled project will not be exported, due to that the fact that we can't re-configure GitHub connections on the destination side, because IDs and other detials are all instance-specific on the GitHub App side.
:::
...
- Library step templates
  - In addtion to the ID needing to be update, the template version also needs to be updated
