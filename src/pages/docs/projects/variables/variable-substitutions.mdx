...
Octopus supports basic arithmetic on variable values using the `calc` statement, so you can derive a value like an IP offset or a scaled count without pre-computing it yourself. Five operators are supported: addition (`+`), subtraction (`-`), multiplication (`*`), division (`/`), and modulo (`%`).
...
- `#{calc 22 % ScaleFactor}` evaluates to `10`
- `#{each i in Numbers}#{calc i + 5}#{/each}` evaluates to `15 25 35 45 55`

When a variable appears on the left-hand side of a divide or subtract operation, enclose its name in braces so the operator symbol isn't parsed as part of the variable name: `#{calc {IPOffset[Primary]} - 4}`. Modulo doesn't need this: `%` can't appear in a variable name, so `#{calc ScaleFactor % 5}` parses unambiguously without braces.

Modulo operates on whole numbers only, unlike the other four operators. If either operand isn't a whole number, or the right-hand operand is `0`, the expression has no result and is left unevaluated, appearing in the output as written — the same as an undefined variable.
...
- [PLACEHOLDER: the self-hosted Octopus Server version the modulo (`%`) `calc` operator ships in — needs: confirmation of the release this ships in]
