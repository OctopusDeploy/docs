...
Tenant variables allow you to configure [variables](/docs/projects/variables/) that can have different values for each [tenant](/docs/tenants)/environment combination. Tenant variables are provided using variable templates which map a single variable name to different values for each tenant based on what you choose to set.
...
The variable value for each tenant can be set in the same way as the project template values. Click the **Edit** icon on the right-side of a table row, update the value, then click **Update** to save the new value. Since the same library variable set may be included in multiple projects, any change to a tenant's value will also affect other projects the tenant is connected to.
