...
:::div{.warning}
From Octopus version **2025.4.3897** we support tagging environment. This functionality is behind the `Extended Tag Sets` feature toggle, to request this functionality early, please contact [support](https://octopus.com/support).
:::

You can apply tags to environments to classify them with custom metadata. This allows you to:

- Classify environments by attributes like cloud provider, region, or tier.
- Filter environments by tags.

To use tags with environments, you need to create tag sets with the **Environment** scope. Tags are referenced using their canonical name format: `TagSetName/TagName`. For FreeText tag sets, the tag set name must match exactly, but the tag value can be any arbitrary text, such as `Region/us-west-2` or `Region/custom-value-123`.
