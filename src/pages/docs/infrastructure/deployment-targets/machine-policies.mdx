...
:::div{.hint}
Configurable machine package cache retention was introduced in **Octopus 2025.3**. Support for this feature is provided via the feature toggle, `OCTOPUS__FeatureToggles__ConfigurablePackageCacheRetentionFeatureToggle`.
:::

The machine package cache retention policy ensures that packages are periodically removed from the machine cache. This is handled by Octopus by default, but can be configured per machine policy as required.policy
