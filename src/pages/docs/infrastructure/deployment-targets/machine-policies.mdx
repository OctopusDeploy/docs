...
- **When enabled** it will re-attempt communication with the Tentacle for the amount of time configured. However, after that, continued network errors will cause a failure.

:::div{.warning}
Using this setting with a Tentacle that runs on ephemeral storage (e.g. running Tentacle in Docker without a mount) will introduce the possability that a script could run twice.

It is recommended to run Tentacle on a file system that is persistent.
:::
...
If you would like to retry a particular step within the deployment process for other types of temporary or transient errors, then that can be [configured separately](/docs/projects/steps/conditions#retries-and-execution-timeouts).
