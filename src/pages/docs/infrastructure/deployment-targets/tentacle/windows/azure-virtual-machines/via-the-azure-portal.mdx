...
**Roles**: The [target tags](/docs/infrastructure/deployment-targets/#target-roles) (formerly target roles) to give to the machine. Again, separate them using commas for more than one, for example: `web-server,app-server`.
