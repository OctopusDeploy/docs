...
- Change the server bindings to either HTTP or HTTPS and use the remaining port for polling Tentacle connections.
  - The listening port Octopus Server uses can be [changed from the command line](/docs/administration/octopus.server.exe-command-line/configure) using the `--commsListenPort` option.
...
From Octopus Server release `2026.1` there will be [limited support](https://octopus.com/docs/deprecations#dropping-capability-for-windows-server-2008-workers-and-targets-in-20251) for executing workloads on targets or workers running Windows Server 2008. As the version of [Calamari](https://octopus.com/docs/administration/calamari) shipped with later version of Octopus Server will no longer be compatible with Windows Server 2008, the capability has been made available to pin the version of Calamari used on the target itself.
...
### RHEL 7 Limited Support

Similar to the limited Windows Server 2008 support above, a linux version is available for this older version of Octopus that will work on RHEL 7 using the above manual extraction method. Download this version from [here](https://download.octopusdeploy.com/calamari/Calamari.Legacy.Linux.2026.1.44-dotnet6.zip) ([md5](https://download.octopusdeploy.com/calamari/Calamari.Legacy.Linux.2026.1.44-dotnet6.md5), [sha256](https://download.octopusdeploy.com/calamari/Calamari.Legacy.Linux.2026.1.44-dotnet6.sha256)). Note that this capability will also likely be dropped as a workaround from a future `2026.3` release.
