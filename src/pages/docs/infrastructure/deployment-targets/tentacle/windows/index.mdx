...
From Octopus Server release `2024.1` there will be [limited support](https://octopus.com/docs/deprecations#dropping-capability-for-windows-server-2008-workers-and-targets-in-20251) for executing workloads on targets or workers running Windows Server 2008. As the version of [Calamari](https://octopus.com/docs/octopus-rest-api/calamari) shipped with later version of Octopus Server will no longer be compatable with Windows Server 2008, the capability has been made available to effectively pin the version of Calamari used on the target itself. 

To allow this:
1. Download the [most recent version of Calamari](https://download.octopusdeploy.com/calamari/Calamari.Legacy.27.2.4-ci0003.zip) (currently `27.2.4-ci0003`). This is version of Calamari is unlikely to be updated unless there are significant vulnerbilities discovered so it may be missing new capabilities that are released later than `2025.1`.
2. Extract the zip contents to a location on the target. 
3. Set an environment variable `CalamariDirectoryPath` with a value to the extracted location. This variable should be provided in a context that will be available to the Tentacle process.
4. Restart the Tentacle process to ensure that it has access to this new variable.

The next time a deployment is run, Octopus Server should ignore the embedded Calamari version and instead use the one available on the target. Warnings will continue to be logged to provide a clear signal that this limited support capability should be considered a temporary stop-gap as we are likely to drop support entirely for this workaround from `2026.1`.