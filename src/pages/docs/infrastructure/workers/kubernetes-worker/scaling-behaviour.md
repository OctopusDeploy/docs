...
To achieve this, the worker creates temporary pods for each new deployment task. When there are no deployment tasks running, the Kubernetes worker will shrink to just a couple of pods, consuming minimal resources on your cluster. Any released resources can then be used by other applications on the cluster.
...
Kubernetes decides when to start a pod based on resource *requests*, not usage.
