...
pubDate: 2025-11-05
modDate: 2025-11-05
title: Configuring Microsoft Entra
...
2. On the left side menu of [Azure Portal](https://portal.azure.com) select **All services** and then select **Microsoft Entra ID**

:::figure
![Open Microsoft Entra ID service](/docs/img/security/authentication/oidc-authentication/images/entra-id-service.png)
:::

3. In the top menu, select **Add** then choose **App registration**:

:::figure
![New App registration](/docs/img/security/authentication/oidc-authentication/images/entra-id-new-app-registration.png)
:::

4. Choose a **Name** such as *Octopus Deploy*
...
5. Select the correct **Supported account types** for the accounts that should be allowed access

6. Choose **Web** from the **Select a platform** drop down and enter the **Redirect URI** as `https://your-octopus-url/api/users/authenticatedToken/GenericOidc` (replacing `https://your-octopus-url` with the URL of your Octopus Server)
   ...
   - Include `/api/users/authenticatedToken/GenericOidc` at the end of your Octopus URL.
...
2. Input logout URL and enter `https://your-octopus-url/app#/users/sign-out` substituting your URL.
...
Under the App Registration, you can see the **Application (client) ID** and **Directory (tenant) ID** on your App's Overview page.
...
Under the App Registration, navigate to the **Certificates & secrets** page and click **New client secret** to generate a new client secret for the App registration.
...
:::div{.hint}
If Microsoft Entra ID is used to synchronize external groups with the 'group' role claim type and the user is a member of more than 200 Microsoft Entra ID groups, the **Client Secret** is required.
:::
...
Alternatively, you can set the configuration via the command line. Run the following from a command prompt in the folder where you installed Octopus Server:

```powershell
Octopus.Server.exe configure --genericOidcIsEnabled=true --genericOidcIssuer=Issuer --genericOidcClientId=ClientID --genericOidcClientSecret=ClientSecret

# e.g.
# Octopus.Server.exe configure --genericOidcIsEnabled=true --genericOidcIssuer=https://login.microsoftonline.com/12341234-xxxx-xxxx-xxxx-xxxxxxxxxxxx --genericOidcClientId=43214321-xxxx-xxxx-xxxx-xxxxxxxxxxxx --azureADClientSecret=bCeXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```
