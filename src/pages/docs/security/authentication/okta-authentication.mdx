...
1. Select the Applications tab and click the **Create App Integration** button.
   ...
   ![The Create App Integration button on the Okta Applications tab](/docs/img/security/authentication/okta/okta-add-app.png)
...
2. Choose **Web** for the **OIDC - OpenID Connect** for the **Sign-in method** and **Web Application** for the **Application type** and click the **Next** button.
   ...
   ![Choosing OIDC and Web Application for the new Okta app integration](/docs/img/security/authentication/okta/okta-new-app-integration.png)
...
3. Enter an **App integration name** like Octopus Deploy and for the **Sign-in redirect URIs** enter `https://octopus.example.com/api/users/authenticatedToken/Okta` replacing `https://octopus.example.com` with the public URL of your Octopus Server, remove any default **Sign-out redirect URIs** and click the **Save** button.
   ...
   ![The app integration name and sign-in redirect URI](/docs/img/security/authentication/okta/okta-create-openid-integration.png)
...
:::
   ...
   If you want to allow users to log in directly from Okta then change the **Login initiated by** to *Either Okta or App*, set **Login flow** to *Redirect to app to initiate login*, and set the **Initiate login URI** to `https://octopus.example.com/#/users/sign-in`.

   ![The Okta initiate login settings](/docs/img/security/authentication/okta/okta-initiate-login.png)
...
:::figure
![The OpenID Connect ID Token section showing the Issuer and Audience](/docs/img/security/authentication/okta/okta-openid-token.png)
:::

#### Okta group integration \{#okta-groups}

If you want Okta groups to flow through to Octopus, you'll need to change the *Groups claim* fields as follows:
...
If you want to configure Okta to present your active directory groups, use a similar approach to [Okta group integration](#okta-groups), however we recommend that you use a *Groups claim type* of **Expression** instead of **Filter**.
...
> A complete guide to Okta's group expressions are available [Okta's guide to dynamic group claims](https://developer.okta.com/docs/guides/customize-tokens-dynamic/main/#add-a-groups-claim-with-a-dynamic-allow-list)
...
1. Select the **Assignments** tab and click the **Assign** button. You can assign your app to people, and to groups.
   ...
   ![Assigning the Okta app to users and groups](/docs/img/security/authentication/okta/okta-assign-app.png)
...
:::figure
![The View Logs link in the Okta admin portal](/docs/img/security/authentication/okta/okta-view-logs.png)
:::
...
2. In Chrome Dev Tools this is called "Preserve Log". In Firefox this is called "Persist Logs".
   ...
   ![The Preserve Log option in the browser developer tools network tab](/docs/img/security/authentication/images/5866122.png)
...
4. Grab the contents of the `id_token` field and paste that into [https://jwt.io/](https://jwt.io/) which will decode the token for you.
   ...
   ![An Okta id_token decoded at jwt.io](/docs/img/security/authentication/images/5866123.png)

:::div{.hint}
Don't worry if jwt.io complains about the token signature, it doesn't support RS256 which is used by Okta.
:::

1. Octopus uses most of the data to validate the token, but primarily uses the `sub`, `email` and `name` claims. If these claims are not present you will likely see unexpected behavior.
2. If you are not able to figure out what is going wrong, please send a copy of the decoded payload to our [support team](https://octopus.com/support) and let them know what behavior you are experiencing.
...
3. Attempt to sign into Octopus using Okta and find the HTTP GET that is sent to Okta. Inside of this request you will see request scopes.
   ...
   ![The scopes included in the sign-in request sent to Okta](/docs/img/security/authentication/images/okta-request-scope.png)
