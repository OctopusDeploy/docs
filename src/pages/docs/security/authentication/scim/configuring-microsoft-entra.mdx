...
- [Entra ID authentication](../azure-ad-authentication) configured and working
...
1. Configure [Entra ID authentication](../azure-ad-authentication) in Octopus Deploy. Don't configure any [external group mappings](../azure-ad-authentication#mapping-microsoft-entra-id-users-into-octopus-teams-optional) as SCIM will allow Entra ID to create internal groups within Octopus Deploy.
...
Create a dedicated [Service Account](../../users-and-teams/service-accounts) for Entra ID SCIM, and add it to the **Octopus Managers** team, so that it has sufficient permissions. Once the **Service Account** has been created, create a new API key for the account ready to provide to Entra ID.
...
1. Click **Start provisioning**. This will queue up a job in Entra ID to perform the initial sync, which may take a few minutes to start. Progress is shown on the **Overview** page.
1. Entra ID will now reach out to Octopus Deploy regularly via the SCIM API whenever a user or group needs to be created, updated or deleted.
