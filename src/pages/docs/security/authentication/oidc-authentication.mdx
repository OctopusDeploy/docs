...
Octopus Deploy supports authentication using OpenID Connect (OIDC) from a third-party identity provider, such as:
    - [Azure Directory Federation Services (AD FS)](https://learn.microsoft.com/en-us/windows-server/identity/ad-fs/ad-fs-overview)
...
1. Configure your Octopus Deploy instance to trust and use OIDC as an authentication provider.
1. You can optionally assign users to **Teams** based on the role information provided by your identity provider.
...
1. Navigate to **Configuration ➜ Settings ➜ OpenID Connect** and populate the following fields:
     ...
     - **Scopes** has a default value, but can be configured if your identity provider requires a custom scope. The scope of `groups` is optional can be omitted but the scopes of `openid profile email` are required for Octopus Deploy to retrieve user information from your identity provider.
