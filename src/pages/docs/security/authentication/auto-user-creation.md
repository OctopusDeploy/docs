...
The Active Directory and OpenID connect providers will, by default, automatically create a new user record for any user who can successfully authenticate but is not currently recognized (based on the checks and fallbacks described in [matching external identities to Octopus users](/docs/security/authentication#matching-external-identities)).
