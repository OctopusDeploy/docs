...
2. Add one of the steps called `Octopus - Create Octoterra Space`. Each step indicates the Terraform backend it supports. For example, the `Octopus - Create Octoterra Space (S3 Backend)` step configures a S3 Terraform backend.
   ...
   3. Set the `Octopus Space Managers` field to a comma separated list of team IDs to assign as space managers. Built-in teams like `Octopus Administrator` have named IDs like `teams-administrators`. Custom teams have IDs like `Teams-15`.
