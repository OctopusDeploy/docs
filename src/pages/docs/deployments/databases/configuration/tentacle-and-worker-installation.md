...
:::figure
![Multiple VMs in a single worker pool](/docs/img/deployments/databases/configuration/images/standard-database-worker-pool.png)
:::
...
:::figure
![Workers assigned to more than one pool](/docs/img/deployments/databases/configuration/images/worker-pools-per-usage.png)
:::

To create a new worker pool, go to **Infrastructure ➜ Worker Pools** and then click on the **Add Worker Pool** button:

:::figure
![The Add Worker Pool button under Infrastructure ➜ Worker Pools](/docs/img/deployments/databases/configuration/images/add-worker-pool.png)
:::

In the modal dialog, add the name of the worker pool you wish to add:

:::figure
![The dialog for naming a new worker pool](/docs/img/deployments/databases/configuration/images/add-worker-pool-modal.png)
:::
...
:::figure
![The worker pool maintenance screen](/docs/img/deployments/databases/configuration/images/worker-pool-edit-dialog.png)
:::

When you add a worker to the pool, you are given a choice of listening, polling, and in the case of Linux, SSH:

:::figure
![Choosing between listening, polling and SSH when adding a worker](/docs/img/deployments/databases/configuration/images/add-worker-to-pool.png)
:::
...
:::figure
![The worker pool option in a deployment step](/docs/img/deployments/databases/configuration/images/use-worker-in-deployment-process.png)
:::
...
:::figure
![Environment-specific service accounts for database deployments](/docs/img/deployments/databases/configuration/images/worker-pool-per-environment.png)
:::

To start, create a dedicated Worker pool for each environment:

:::figure
![A dedicated worker pool for each environment](/docs/img/deployments/databases/configuration/images/environment-specific-worker-pools.png)
:::
...
:::figure
![Changing a variable type to Worker Pool](/docs/img/deployments/databases/configuration/images/worker-pool-variable-type.png)
:::

Select the Worker pool:

:::figure
![Selecting the worker pool for the variable value](/docs/img/deployments/databases/configuration/images/worker-pool-variable-type-selection.png)
:::

With that option, you can scope Worker pools to specific environments:

:::figure
![Worker pool variable values scoped to each environment](/docs/img/deployments/databases/configuration/images/worker-pool-variable-per-environment.png)
:::
...
:::figure
![A step set to run on a worker from a pool selected via a variable](/docs/img/deployments/databases/configuration/images/use-worker-pool-variable.png)
:::
...
:::figure
![Creating a cloud region with the DbWorker target tag](/docs/img/deployments/databases/configuration/images/create-cloud-region.png)
:::

When done, you will have a cloud region per environment:

:::figure
![A cloud region for each environment](/docs/img/deployments/databases/configuration/images/environment-cloud-regions.png)
:::
...
:::figure
![A step execution location set to the DbWorker target tag](/docs/img/deployments/databases/configuration/images/cloud-region-execution-location.png)
:::

That step needs to be repeated for each step in the process:

:::figure
![A deployment process where every step targets a cloud region](/docs/img/deployments/databases/configuration/images/process-with-cloud-region-targets.png)
:::
...
:::figure
![A release deployed to Test using the Test Database Worker Region](/docs/img/deployments/databases/configuration/images/release-with-cloud-region.png)
:::
...
:::figure
![A jump box between Octopus Deploy and the database server virtual IP](/docs/img/deployments/databases/configuration/images/database-with-jump-box.png)
:::
...
:::figure
![A jump box in each of two domains](/docs/img/deployments/databases/configuration/images/database-jump-box-multiple-domains.png)
:::
...
![Several Tentacles installed on a single jump box](/docs/img/deployments/databases/configuration/images/database-jump-box-multiple-tentacles.png)
