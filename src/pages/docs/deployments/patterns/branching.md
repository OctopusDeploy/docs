...
Sometimes a new feature introduces a new component requiring a change to the deployment process.  All projects should use project [config-as-code](/docs/projects/version-control) feature.  That stores the deployment process, runbooks, variables, and deployment settings in a git repo.  
...
Store the Octopus Deploy configuration in the same repository as the source code.  Often a change to the deployment process has a corresponding code change.  By using project config-as-code, both code and deployment changes are made in a branch and merged in the same pull request.
...
3. Have a single deployment process, but use [script steps](/docs/deployments/custom-scripts), [output variables](/docs/projects/variables/output-variables), and [variable run conditions](/docs/projects/steps/conditions/#variable-expressions) to skip unnecessary steps.  The script steps run on the deployment targets and pre-check for version differences.
