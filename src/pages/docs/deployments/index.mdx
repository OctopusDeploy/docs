...
### Steps
Octopus provides a library of [pre-built step templates](/docs/projects/steps) for deployment tasks, offering a simplified configuration experience by separating scripts and API calls. 
...
[Guided failure](/docs/releases/guided-failures) mode pauses the deployment when a step fails and allows you to retry the failed step (after fixing any errors if required), skip it, or fail the deployment.
...
### Variables
Octopus [Variables](/docs/projects/variables/) allow you to apply the correct variables easily during a deployment. Octopus can manage simple values, secrets, and accounts as variables.
...
Use the navigation to discover deployment examples for different types of applications and technologies using Octopus. It also includes a number of [common deployment patterns and practices](/docs/deployments/patterns).