...
:::div{.hint}
Configurable machine package cache retention was introduced in **Octopus 2025.3** to optionally allow configuring Octopus to keep packages by quantity. Support for this feature is provided via the feature toggle, `OCTOPUS__FeatureToggles__ConfigurablePackageCacheRetentionFeatureToggle`.
If `OCTOPUS__FeatureToggles__UseQuantityPackageCacheRetentionFeatureToggle` is also enabled, Octopus will default to managing the package cache by quantity rather than disk space.
:::

##### Free disk space

Packages that are transferred during the deployment are managed based on free disk space. For each deployment, prior to acquiring the package, the free disk space is checked and if less than 20% it will delete the “least used” package(s) from the Tentacle file store location. Packages that get removed will be reacquired if needed for another deployment.
