...
1. configure any volume mounts or symbolic links for file storage.
...
1. Wait for all the tasks to complete.  Failure to do so will cause those tasks to fail.
1. Delete the application host.

The complexity for removing a node is due to having to invoke the API to drain the node and waiting for the node to complete any in-flight tasks.  For cloud providers such as Azure or AWS that typically means leveraging a function or a Lambda.  

For scripts and examples, please refer to the [auto-scaling high availability nodes page](/docs/administration/high-availability/configure/auto-scaling-high-availability-nodes).
