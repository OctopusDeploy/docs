...
import Helm from 'src/components/icons/Helm.astro';
...
A clickable surface for a title, an optional description, and an optional image or icon, all leading to a single destination.
...
4. **icon** / **imgSrc** + **imgAlt**: Optional, and mutually exclusive — pass one or the other, or neither. **icon** takes a component, for a design-system icon; **imgSrc** takes a URL, with **imgAlt** for its alt text.
5. **`featured` variant**: Only used on home page to emphasise featured content.
...
<summary>No custom element</summary>
...
<Card
  description="Only use on the homepage to feature content. Keep the description brief, up to two sentences and maximum four lines."
  link="/docs/deployments/kubernetes"
...
<Card
  ...
  icon={Helm}
  link="/docs/deployments/kubernetes"
  title="Featured Icon Example Title"
  variant="featured"
/>
