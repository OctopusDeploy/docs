...
import ApiBadge from 'src/components/ApiBadge.astro';
...
The ApiBadge component marks a piece of documentation with the HTTP method it
describes. Each method carries its own icon and colour, in both light and dark
themes.

1. **method**: Required: `get`, `post`, `delete`, or `put`. It sets the icon, the
   colours and the label text.
2. **hasLabel**: Optional `true` (default) / `false`. Set it to `false` for the
   icon-only badge, which keeps the method name for screen readers while hiding
   it visually.

Any other attribute is passed through to the rendered `<span>`, so a `class` of
your own reaches the badge and you can position it from the parent component.
Astro scopes each component's `<style>` to its own markup, so a rule aimed at
that class needs `:global()` to cross into ApiBadge.
...
<div class="docs-home simple-grid">
  <div class="btn-row">
    <ApiBadge method="get" hasLabel={false} />
    <ApiBadge method="post" hasLabel={false} />
    <ApiBadge method="delete" hasLabel={false} />
    <ApiBadge method="put" hasLabel={false} />
  </div>
  <div>
    ```
    <ApiBadge method="get" hasLabel={false} />
