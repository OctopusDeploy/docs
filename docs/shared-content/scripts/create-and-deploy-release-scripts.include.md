...
try {
    ...
    $template = Invoke-WebRequest -Uri "$octopusSpaceUrl/deploymentprocesses/deploymentprocess-$($project.id)/template" -Headers $headers -ErrorVariable octoError | ConvertFrom-Json
