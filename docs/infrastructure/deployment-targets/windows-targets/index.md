...
use the remaining port for polling Tentacle connections. The listening port Octopus Server uses can be [changed from the command line](/docs/installation/octopus.server.exe-command-line/configure.md) using the `--commsListenPort` option.
