...
If your Octopus Deploy instance is having problems trying to connect with your Docker Registry when runing the `Save and Test` operation, it may failing from from several reasons outside the control of Octopus Deploy. 

Try to connect to your registry directly through the browser from the same machine that Octopus is hosted on. Use the feed url you provided and esure that either `/v1` or `/v2` is appended to the path depending on what version of the Docker Registry API you are running. If the connection is valid then you should recieve a `200` response, possibly recieving a user auth request (see API details above under [Docker Registry API version discovery](#DockerRegistriesasFeeds-VersionDiscovery)). If this does not occur then you may be having issues with your registry which you may need to fix before using through Octopus Deploy.
