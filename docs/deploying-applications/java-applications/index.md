...
The package filenames have to adhere to the standard Octopus [versioning rules](/docs/packaging-applications/creating-packages/versioning.md), which are based on the SemVer format.
...
The Java deployment steps include a number of unique error codes that may be displayed in the output if there was an error. Please see the [error codes and troubleshooting](/docs/deploying-applications/java-applications/error-messages.md) page for more details.
