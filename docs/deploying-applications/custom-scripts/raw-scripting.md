...
1. Add a [Transfer A Package](/docs/deploying-applications/deployment-process/deploying-packages/transfer-package.md) step and provide a  location that it should be moved to as part of the deployment. In this case we will transfer it to `~/temp/uploads`.  Note that this directory will be created if it does not already exist. Give the step the name *Transfer AcmeWeb* and Include the relevant role for your SSH target.
