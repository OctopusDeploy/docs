...
To deploy a Windows Service, add a *Deploy a Windows Service* step. For information about adding a step to the deployment process, see the [add step](/docssteps/index.md) section.

![](/docs/images/5671696/5865909.png "width=170")

:::hint
**Pre Octopus 3.4.7**
The *Deploy a Windows Service Step* was introduced in Octopus version **3.4.7**. Prior to this Windows Services were deployed by enabling the *Windows Service* feature on a [Deploy a Package Step](/docsdeploying-packages/index.md).
...
Out of the box, Octopus will do the right thing to deploy your Windows Service, and the conventions we have chosen will eliminate a lot of problems with file locks, and leaving stale files behind. By default Octopus will follow the conventions described in [Deploying packages](/docsdeploying-packages/index.md) and apply the different features you select in the order described in [Package deployment feature ordering](/docs/reference/package-deployment-feature-ordering.md).
...
9. [Output variables](/docsvariables/output-variables.md) and deployment [artifacts](/docsartifacts.md) from this step are sent back to the Octopus Server
