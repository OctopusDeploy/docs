...
- Which [Lifecycle](/docsprojects/lifecycles/index.md) to use for promoting Releases: for example, feature releases may be promoted through the testing environments, while hot-fix releases may go directly to production.
...
- Which [Variables](/docsvariables/index.md) to use: Variables can be scoped to channels.
...
Version rules assist in selecting the correct versions of packages for the Channel.  They are only used when creating a release, either manually or via [Automatic Release Creation](/docssteps/automatic-release-creation.md).
...
The package list allows you to select the version of each package involved in the deployment.  The *latest* column displays the latest packages that match the version rules defined for the Channel (see [version rules](/docschannels.md) for more information).
...
When enabling [Automatic Release Creation](/docssteps/automatic-release-creation.md) for your project, you are required to select a Channel (if the project has more than one).
