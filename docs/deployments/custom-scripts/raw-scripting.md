...
The bootstrapping script that is provided by Calamari will not be available and so you will lose the ability to use helper functions like [new\_octopusartifact](/docs/deployment-process/artifacts.md), [set\_octopusvariable](/docs/projects/variables/output-variables.md) or [get\_octopusvariable](/docs/deployments/custom-scripts/index.md). You can still use the standard **#{MyVariable}** variable substitution syntax however since this is replaced on the server, environment variables from your target will not be available through Octopus variables.
...
![Machine policy settings for connection test only](images/machine-policy-connection-test-only.png "width=500")
...
1. Add a [Transfer A Package](/docs/deployments/package-deployments/transfer-package.md) step.
