...
In **Octopus 3.0**, you can configure an [Offline Package Drop deployment target](/docs/infrastructure/deploymnent-targets/offline-package-drop.md). This allows you to "deploy" to a location on the filesystem and take that deployment offline to be used elsewhere. The dropped package contains everything you need to deploy to a location offsite.
