...
- SSH targets using a [Key Pair](/docs/infrastructure/deployment-targets/ssh-targets/ssh-key-pair.md) where the key is protected by a passphrase** are affected by FIPS, however we still consider SSH communication to be secure:
