...
- SSH targets using a [Key Pair](/docs/infrastructure/accounts/ssh-key-pair.md) where the key is protected by a passphrase** are affected by FIPS, however we still consider SSH communication to be secure:
