...
The examples in this guide can be used with an AWS AMI instance of Ubuntu 14.04 LTS or an Azure VM running Ubuntu 14.04 LTS. Different bases instances may require different configuration steps.
...
To help manage the web process when it runs on the machine, we will install the [pm2 library](https://github.com/Unitech/pm2). The pm2 library is a node module and we will install it as a global module. This library is available in the **npm** repository which, if not already available, will also need to be installed:
...
To avoid confusion with the other *octo* themed npm packages and to provide guidance on those supported directly by Octopus Deploy, we published these packages as [scoped packages](https://docs.npmjs.com/misc/scope). 
...
Use the following command to verify which version you're currently running:
...
## Configure Publish Task {#CreatePushNode.jsProject-ConfigurePublishTask}
...
Note, your are packaging the resources that are needed to run the application and the **node\_modules** directory. You can ignore source and build files. We recommend including **node\_modules** directory so that you are not relying on external repositories to return the same packages during deployment that you used during development.
...
We will also install [grunt-contrib-clean](https://github.com/gruntjs/grunt-contrib-clean) library to clean the build output directory to avoid pushing the same packages on each publish:
...
2. Navigate **{{Infrastructure,Environments}}**, select the **prod** environment, click **Add deployment target**, and select *SSH Connection*.
3. Enter the IP or DNS of the machine that is accessible to the Octopus Server. *In our case below it's the public IP provided by Azure/AWS.*
4. Click *Discover* to automatically pre-populate the SSH fingerprint for the remote server.
5. Continue to fill out the rest of the details, selecting the account that you created above.
...
2. Give the new project an appropriate name, save the project, and navigate to the project's *Process* page and click **{{Add Step,Deploy a Package}}**.
 > Ensure that the target role matches that was assigned to the machine in the previous step and select *octofxjs* as the Package ID. This Package ID is derived from the first section of the package that was previously uploaded.
...
2. Disable the two configuration steps that are already selected.
...
5. Navigate to the **Variables** tab and add a new variable named **projectVariable** with some text to appear underneath the title bar on the web page, but leave the variable un-scoped, and click **Save**.
