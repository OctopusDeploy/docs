...
|`Octopus.Action.SkipRemainingConventions` <br/>If set by the user, completes processing of the action without runnning further conventions/scripts *(Boolean)*. (e.g. `Set-OctopusVariable -name 'Octopus.Action.SkipRemainingConventions' -value 'True'`) | *True*|
