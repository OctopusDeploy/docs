...
Octopus Deploy is a deployment tool.  It takes the packages and artifacts generated by your build server and deploys them to a variety of targets, be it Windows, Linux, Azure, AWS, or Kubernetes, in a safe and consistent process.  This section provides a conceptual overview of Octopus Deploy and provides step-by-step instructions to help you master the fundamentals of Octopus Deploy whether you are using the self-hosted Octopus Server or Octopus Cloud.  

## Hello World

The best way to learn something new is hands-on interaction.  One of the first tasks when learning a new programming language is to create something that prints "hello world."  By the end of this guide you will learn the fundamentals on how to:
...
Before configuring a hello world deployment you need to setup Octopus Deploy.  You have a choice of three.

1. Octopus Cloud -> we setup and host everything for you.  Learn more about [getting started](https://octopus.com/start/cloud).
...
Don't forget, you will need a license to self-host Octopus Deploy.  If you are just starting out, we offer a [free license](https://octopus.com/start/server).
...
1. Scroll down to the **Script**, select your script language of choice enter the following script in the **Inline Source Code** section:
...
Because we didn't define any deployment targets for the **Test** environment, Octopus leased a dynamic worker (a machine that execute tasks on behalf of the Octopus Server) that was then used to execute the hello world script.  If you are on a self-hosted instance of Octopus Deploy, you won't see that message.
...
Octopus allows you to define variables and scope them for use in different phases of your deployments.  This allows you to have a consistent deployment process across your infrastructure wihtout having to hard-code or manually update configuration settings that differ across environments, deployment targets, channels, or tenants.
...
The variables can be referenced by steps in the deployment process.
...
Approvals in Octopus Deploy are accomplished by the manual intervention step.  A deployment will pause when a manual intervention step is encountered and wait for a approval or a rejection from a member of the specified team.
...
1. Reorder the steps so the manual intervention is the first one in the list.
1. Click on **DONE** and then click on **SAVE**.

Create a new release and deploy it through to **Production**.  You will see the approval step being skipped in *Development* and *Testing*.  When you deploy to **Production** the deployment will pause and you will need to approve it prior to continuing.
...
As you configure your deployment targets, select the environment they will belong to, and assign the target role(s).  This ensures you deploy the right software to the right deployment targets. Typical target roles include:
...
### Deploy a package to a deployment target
...
1. Leave the **Step Name** and the **Enabled** checkbox as is.
...
Create a new release and deploy it to *Development*.  This will deploy the *hello world* package to *dev-server-01*.
...
This guided covered the basics of Octopus Deploy.  There are still many more concepts to familiarize yourself with.
