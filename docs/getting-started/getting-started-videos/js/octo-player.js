...
var octo = (function($, undefined) {
  'use strict';

  function octo(el, api_key, options) {
  ...
  octo.prototype = {
    ...
    init: function(el, api_key, options) {
      ...
      this.DOM = {};
      this.DOM.$el = $(el);
      this.DOM.$playlc = this.DOM.$el.find('.octo-playlist');
      this.DOM.$items =  this.DOM.$el.find('.octo-items');
      this.DOM.$videoc = this.DOM.$el.find('.octo-video');
      this.DOM.$title  = this.DOM.$el.find('.octo-title');
      this.DOM.$desc   = this.DOM.$el.find('.octo-desc');
      ...
      this.DOM.$el.attr('data-octo',this.data.player_uid).find('.octo-video-player').attr('id','octo-vp-'+this.data.player_uid).attr('name','octo-vp-'+this.data.player_uid);
      if (this.options.debug) console.log('Unique ID: octo-vp-'+this.data.player_uid);
    ...
    addAPIPlayer: function() {
      ...
      this.DOM.$el[0].ytplayer = new YT.Player('octo-vp-'+that.data.player_uid, {
        // height: '390',
        // width: '640',
        playerVars: {
          // controls: 0,
...
function onYouTubeIframeAPIReady() {
  ...
  $('[data-octo]').each(function(idx, el) {
    $(el)[0].octo_data_obj.onYTIframeAPIReadyCallback();
  });
}

// JQuery hook
$.fn.octo = function(api_key, options) {
  return this.each(function() {
    // Store object in DOM element
    this.octo_data_obj = new octo(this, api_key, options);
  });
};
