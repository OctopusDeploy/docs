...
Deployment and runbook processes are the series of steps you specify to either deploy or manage your application.  The difference between a deployment and runbook is the intended usage:
...
The task log represents the details of the deployment, but is typically only useful when debugging a deployment.  The majority of the time, you'll spend in the Task Summary.  This will provide an overview of the deployment or runbook run.
...
It is important to understand the difference between artifacts and the task logs.
...
- Artifacts are files generated by a specific step containing useful data for users to download and review.  Examples include test run results, delta script reports, and configuration files.

Both the task log and artifacts can be downloaded, there is no need to create artifacts from the log messages.

## Logging

You can never have too much logging when are running custom PowerShell, Python, or Bash scripts in your deployment or runbook process.  
...
- Backing up a database prior to deployment.
- Creating a test database after a deployment for developers to use locally.

## Consistent Processes for all environments

The deployment and runbook processes should be the exact same for all environments, except for a step or two is enabled or disabled.  In other words, if you are deploying to Kubernetes cluster in **production**, you should deploy to a Kubernetes cluster in your testing environments.  Don't deploy to VMs in your testing environments and then to something else in your **production** environments.  Having different destination architectures means there is no guarantee of knowing if the process will work until you get to **production**.

## Use Output Variables to control flow

When defining your runbook or deployment process you can configure steps a run condition based on a variable.  Generally that feature is used to skip over a specific step when a certain condition occurs.  For example, skip over a testing step if only .css or image files were deployed.
