...
4. Does your tool chain support the versioning scheme? *For example: Octopus Deploy supports Semantic Versioning, which enables enhanced features like [Channels](/docs/deploying-applications/deployment-process//channels.md).*
...
When it comes to application versioning however, we suggest the "pre-release tag" (the bit after the `-`) can be used for whatever you want. For example: you could build version `1.5.2-rc` of your application, and configure a [Channel](/docs/deploying-applications/deployment-process//channels.md) to promote packages like `*-rc` to Staging and eventually Production.
